...
class ConsultationControllerTest {
    ...
    public void calling_assignForConsultation_with_valid_test_request_id_should_update_the_request_status(){
        ...
        // Create another object of the TestRequest method and explicitly assign this object for Consultation using
        // assignForConsultation() method
        TestRequest testRequestMock= new TestRequest();
        testRequestMock.setRequestId(testRequest.getRequestId());
        testRequestMock.setStatus(RequestStatus.DIAGNOSIS_IN_PROCESS);
        // from consultationController class. Pass the request id of testRequest object.
        when(testRequestUpdateService.assignForConsultation(testRequest.getRequestId(),user)).thenReturn(testRequestMock);
        //Use assertThat() methods to perform the following two comparisons
        //  1. the request ids of both the objects created should be same
        consultationController.assignForConsultation(testRequest.getRequestId());
        assertEquals(testRequest.getRequestId(),testRequestMock.getRequestId());
        //  2. the status of the second object should be equal to 'DIAGNOSIS_IN_PROCESS'
        assertThat(testRequestMock.getStatus(),equalTo(RequestStatus.DIAGNOSIS_IN_PROCESS));
    ...
    public void calling_assignForConsultation_with_valid_test_request_id_should_throw_exception(){
        ...
        assertThat(result.getMessage(),containsString("Bad Request"));
